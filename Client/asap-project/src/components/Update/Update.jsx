...
function Update() {
  ...
  useEffect(() => {
    axios
      .get(`https://calf-kings.onrender.com/players/` + id)
      .then((res) => {
  ...
  const onSubmit = async (formData) => {
    axios
      .put(`https://calf-kings.onrender.com/updateCard/` + id, formData)
      .then(() => {
        navigate("/");
  ...
  return (
    <div className="form-container">
      <form className="form" onSubmit={handleSubmit(onSubmit)}>
        ...
        />
        {errors.calf_ratings && (
          <p className="error">Calf Ratings must be between 1 and 10</p>
        )}
        ...
        />
        {errors.img_url && (
          <p className="error">Valid url starts with "http://" or "https://"</p>
        )}
